...
def compare_and_visualize(overall_rets: dict[str, List]):
    ...
    plt.legend()
    plt.grid()
    plt.savefig(f'strategy/additional_tests/performance_outputs/performance_comparison_chart_{datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}.png')
    plt.clf()
...
def visualize_stock_positions(portfolio_history, smoothen_days=20):
    ...
    plt.legend()
    plt.grid()
    plt.savefig(f'strategy/additional_tests/performance_outputs/stock_positions_chart_{datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}.png')
    plt.clf()
